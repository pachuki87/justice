/**
 * PRUEBAS COMPLETAS DE SEGURIDAD XSS - JUSTICE 2
 * 
...
class XSSSecurityTestSuite {
    ...
    testInputValidation() {
        ...
        testCases.forEach((testCase, index) => {
            const result = XSSProtection.validateInput(testCase.input, testCase.type);
    ...
    generateReport() {
        ...
        require('fs').writeFileSync(
            'xss-security-test-report.json', 
            JSON.stringify(reportData, null, 2)
        );

        console.log('\n💾 Reporte guardado en: xss-security-test-report.json');
